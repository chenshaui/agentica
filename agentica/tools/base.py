...
def get_function_call(
    ...
    if arguments is not None and arguments != "":
        ...
        # Fail closed when Layer 1 dropped a payload. The marker is a JSON
        # object, not a string, so it cannot be a ``send_message`` body or a
        # shell command. Executing it anyway has happened: ``execute`` ran it
        # as shell, ``write_file`` wrote it, ``send_message`` delivered it to
        # a peer as the entire message. Imported here: ``agentica.compression``
        # pulls ``utils.tokens``, which imports this module.
        ...
        if omitted:
            fields = ", ".join(omitted)
            logger.warning(
                f"Refusing {function_to_call.name}: argument(s) [{fields}] "
                f"were dropped from context"
            )
            function_call.arguments = None
            function_call.error = (
                f"Error: argument(s) [{fields}] were dropped from context, so "
                f"there is no payload to execute. The tool was NOT executed. "
                f"Do not pass the eviction marker as the value. For a file "
                f"write, re-read the file and send the real content. For a "
                f"peer message, write a new message from what you still know "
                f"— the omitted text cannot be recovered."
            )
            return function_call
    return function_call
